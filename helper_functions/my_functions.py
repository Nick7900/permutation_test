...
def plot_histograms_old(matrix):
    num_figures = matrix.shape[1]

    for i in range(num_figures):
        permutation_mean = matrix[:, i]
        observed_statistic =matrix[0, i]
        plt.figure(i+1)
        sns.histplot(permutation_mean, kde=True)
        plt.axvline(x=observed_statistic, color='red', linestyle='--', label='Observed Statistic')
        plt.xlabel('Permutation mean')
        plt.ylabel('Density')
        plt.title('Distribution of Permutation Mean')
        plt.legend()
        plt.show()

    plt.show()
    
def plot_histograms(test_statistic):
    plt.figure()
...
def plot_scatter_with_labels(p_values, alpha=0.05):
    """
    Generate a scatter plot with labels based on a binary mask of p-values.
    ...
    Returns:
        None
    """
    # Create a binary mask based on condition (values above alpha)
    mask = p_values < alpha
    ...
    # Add error bars to visualize uncertainty or confidence intervals
    #error_values = np.random.uniform(0.01, 0.05, size=len(p_values))
    #ax.errorbar(np.arange(0, len(p_values)), -np.log10(p_values), yerr=error_values, fmt='none', color='gray', capsize=3)
    ...
    # Set axis limits to focus on relevant data range
